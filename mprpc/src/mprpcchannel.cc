                          ...
                          google::protobuf::Closure* done)
{
    ...
    int clientfd = socket(AF_INET, SOCK_STREAM, 0); // 可以用智能指针创建一个clientfd，自定义删除器，动态维护clientfd
    if(clientfd == -1)
    {
        char errtext[512] = {0};
    ...
    // 反序列化rpc调用的响应数据
    // std::string response_str(recv_buf, 0, recv_size); // bug出现问题，recv_buf中遇到\0导致后面遇到的数据就存不下来了，导致妇女序列化失败
